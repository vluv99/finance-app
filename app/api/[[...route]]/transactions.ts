  ...
  .get(
    ...
    async (c) => {
      ...
      const data = await db
        ...
        .from(transactions)
        .innerJoin(accounts, eq(transactions.accountId, accounts.id)) // inner join cuz we know id has to be available
        .leftJoin(categories, eq(transactions.categoryId, categories.id)) // left join cuz categoryId can be null, so it shouldn't break
        .where(
          and(
